...
services:
  androidsdk:
    ...
    environment:
      ...
      DISPLAY: $DISPLAY
      GRADLE_HOME: $SDK/gradle-8.14.3/bin
      GRADLE_USER_HOME: /home/$USER/.gradle
      PATH: "/home/$USER/app:$SDK/build-tools/36.0.0:$SDK/gradle-8.14.3/bin:$SDK/kotlinc/bin:$SDK/emulator:$SDK/cmdline-tools/latest/bin:$SDK/platform-tools:\$PATH"
...
EOF
  ...
  docker compose run --rm androidsdk sh -c "wget https://services.gradle.org/distributions/gradle-8.14.3-bin.zip && \
    unzip gradle-8.14.3-bin.zip -d sdk && \
    rm gradle-*-bin.zip"
